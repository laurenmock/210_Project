...
First we take out survival outcome and relapse


```{r}
bone2 <- bone %>% dplyr::select(-c(survival_status, relapse)) %>% drop_na()
```
...
```{r}
library(MASS)
full_model <- lm(survival_time ~ ., data = bone2)
...
Forward selection: Has all the predictors but the only significant predictors are:

stem_cell_source and extensive_chronic_GvHD
...
Let's try a survival analysis with the variables from the backwards model since there are fewer variables and a higher adjusted R^2
```{r}
bone
cox_obj <- Surv(bone$survival_time, bone$survival_status)
cox_model <- coxph(cox_obj ~ as.factor(gender_match), as.factor(stem_cell_source), CD34_x1e6_per_kg...CD34kgx10d6, PLT_recovery, as.factor(extensive_chronic_GvHD), data = bone, ties = "exact")
...
In one of our secondary questions, we ask if one of sources of bone marrow (either from peripheral stem cell or bone marrow stem cell) has a greater chance of survival than the other. We can use a survival analysis model to calculate the hazard rate with stem cell source being our variable. 
...
```{r}
cox_mod_source <- coxph(cox_obj ~ as.factor(stem_cell_source), data = bone, ties = "exact")
