...
plot(KM.fit2, xlab = "Days after Transplant", ylab = "Survival Probability", 
     ...
     main = "Kaplan-Meier Survival Curves by Stem Cell Source", lwd = 2)
...
plot(KM.fit4, xlab = "Days after Transplant", ylab = "Survival Probability", mark.time = TRUE, conf.int=, 
     ...
     main = "Kaplan-Meier Survival Curves by CD3+ Dosage per kg")
legend(x = 1400, y = 1, 
...
```{r}
cox_adj3 <- coxph(survival.obj ~ as.factor(stem_cell_source) + CD3_x1e8_per_kg + recipient_age + as.factor(CMV_status), 
                        data = bone, ties = "exact")
summary(cox_adj3)
```

adding CMV status changes stem cell source coefficient less than 10%



**final model = adjusted model 1**
