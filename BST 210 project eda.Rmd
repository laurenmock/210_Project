...
fill this in later...not sure the best way to include this (not worth fixing the whole bulleted list here)
...
fill this in with more information!
...
Well, it turns out there are some other psuedo "missing" values. Some time-to-event variables have entries of 1,000,000, which doesn't make sense in this context.

To illustrate, let us see the number of occurrences of the value 1000000. This value frequently occurs in the data amongst three variables. It seems as though these large values indicate that the event never occured for that subject, but we will need to confirm this. If our assumption is correct, then we will need to seek out advice on how to proceed. 
...
```
So only these three columns have > 5% missing values. We will need to investigate these missing values later if we choose to use these variables in our analysis.
...
### Now look at recipient body mass, rh factor (+ or -), and risk group
...
#### Look at donor/recipient match variables

**From here we can explore variables that track the match between the donors and recipients (match) and make note of the potential multicollinearity here.** 
...
1. Change survival to binary (instead of numeric)
```{r, warning=FALSE}
...
### Now compare outcome variable (survival_status) and numeric variables
