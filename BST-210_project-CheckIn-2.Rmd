...
confusionMatrix(data = as.factor(prediction), reference = as.factor(predictors$survival_status))
```
...
confusionMatrix(data = as.factor(prediction), reference = as.factor(recip_complete$survival_status))
...
Again, we are seeing that are model is not doing to well in terms of accuracy of prediction. However, the model for stem cell source is doing well in terms of sensitivity, unfortunately this is offset by its specificiity. 
...
confusionMatrix(data = as.factor(prediction), reference = as.factor(cases_comp$survival_status))
```
